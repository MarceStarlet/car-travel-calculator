...
public class TravelCalculatorService extends AbstractTravelService {
    ...
    protected static final Graph<String,String,Integer> graph = new Graph<>();
    {
        graph.add("A","B", 5);
        graph.add("A","C", 3);
        graph.add("A","D", 10);
    ...
    protected boolean validateLocation(String location){
        // exists in the graph
        return graph.get().containsKey(location);
    }
    ...
    protected String calculateShortestPath(String key){
        ...
        //synchronized (this) {
            bfsQueue.add(new Node<String, Integer>(key, 0));
    ...
    protected boolean isCarInVertex(String key){
        // simulation of a check car/vertex table or service
        if ( carSimulator.contains(key) ) return true;
        return false;
    }
